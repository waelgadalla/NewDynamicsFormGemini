...
public class SchemaValidationService : ISchemaValidationService
{
    ...
    public IEnumerable<ValidationIssue> ValidateField(FormFieldSchema field, FormModuleSchema module)
    {
        ...
        // Fields should have labels (except Divider)
        ...
        // TypeConfig validation for AutoComplete
        ...
        // TypeConfig validation for FileUpload
        if (field.FieldType == "FileUpload" && field.TypeConfig is FileUploadConfig fu)
        {
            if (fu.AllowedExtensions is null || fu.AllowedExtensions.Length == 0)
            {
                issues.Add(new ValidationIssue(
                    field.Id,
                    ValidationSeverity.Warning,
                    "No File Extensions",
                    "File upload should specify allowed extensions"
                ));
            }
        }
